...
AUTH_RESPONSE=$(curl -L -b $COOKIE_FILE -c $COOKIE_FILE -i "http://localhost:8080/auth-api/authorize?${QUERY_PARAMS}")
echo "$AUTH_RESPONSE"
echo -e "\n"
...
REF_RESPONSE=$(curl -X POST http://localhost:8080/auth-api/token \
 -H "Content-Type: application/x-www-form-urlencoded" \
...
CLI_RESPONSE=$(curl -X POST http://localhost:8080/auth-api/token \
  -H "Content-Type: application/x-www-form-urlencoded" \
